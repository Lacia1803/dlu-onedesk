...
function addWorkingHours(startDate: Date, hoursToAdd: number): Date {
  ...
  while (minutesRemaining > 0) {
    ...
    if (currentMinutesFromMidnight < morningStart) {
      ...
      currentMinutesFromMidnight >= morningStart &&
      currentMinutesFromMidnight < morningEnd
    ) {
      // Đang trong ca sáng
      ...
      currentMinutesFromMidnight >= morningEnd &&
      currentMinutesFromMidnight < afternoonStart
    ) {
      // Giờ nghỉ trưa (11:30 -> 13:00) → nhảy tới 13:00 cùng ngày
      current.setHours(13, 0, 0, 0);
    } else if (
      currentMinutesFromMidnight >= afternoonStart &&
      currentMinutesFromMidnight < afternoonEnd
    ) {
      // Đang trong ca chiều
...
export const REOPEN_TRANSITION: [TicketStatus, TicketStatus] = [
  TicketStatus.CLOSED,
  TicketStatus.OPEN,
];
