  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-xl">
      <div>
        ...
        <Input
          placeholder="Microsoft Office, Visual Studio Code..."
          disabled={loading}
          {...register("name")}
        />
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          {errors.version && (
            <p className="text-sm text-destructive">{errors.version.message as string}</p>
          )}
        </div>
        <div>
          <label className="text-sm font-medium">Giấy phép</label>
          <Input
            placeholder="MIT, Commercial, Free..."
            disabled={loading}
            {...register("license")}
          />
          {errors.license && (
            <p className="text-sm text-destructive">{errors.license.message as string}</p>
          )}
        </div>
      </div>
