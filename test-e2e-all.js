...
(async () => {
  ...
  await check(page, "Dashboard render stats cards", async () => {
    await page.waitForFunction(
      () =>
        document.body.innerText.includes("Tổng thiết bị") ||
        document.body.innerText.includes("thiết bị") ||
        document.body.innerText.includes("SYSTEM.ADMIN_DASHBOARD"),
      { timeout: 10000 }
    );
  });
  await check(page, "Nút 'Xuất báo cáo' (Excel/CSV) tồn tại", async () => {
    await page.waitForFunction(() =>
      Array.from(document.querySelectorAll("button")).some((b) =>
        b.textContent.includes("Xuất báo cáo")
      )
    );
  });
  await check(page, "Nút 'Xuất KPI PDF' tồn tại", async () => {
    await page.waitForFunction(() =>
      Array.from(document.querySelectorAll("button")).some((b) =>
        b.textContent.includes("Xuất KPI PDF")
      )
    );
  });
  await check(page, "Nút 'Gửi reminder quá hạn' tồn tại", async () => {
    await page.waitForFunction(() =>
      Array.from(document.querySelectorAll("button")).some((b) =>
        b.textContent.includes("Gửi reminder")
      )
    );
  });
  await check(page, "Biểu đồ KPI kỹ thuật viên hiển thị", async () => {
    await page.waitForFunction(
      () =>
        document.body.innerText.includes("TECH_KPI_PERFORMANCE") ||
        document.body.innerText.includes("KPI"),
      { timeout: 10000 }
    );
  });
  ...
  await check(page, "Checkbox chọn tất cả hoạt động", async () => {
    ...
    if (thCheckbox) {
      ...
      await page.waitForFunction(
        () =>
          document.body.innerText.includes("Đã chọn") || document.body.innerText.includes("ticket"),
        { timeout: 5000 }
      );
    }
  });
  ...
  await check(page, "Trang FAQ load", async () => {
    await page.waitForFunction(() => document.body.innerText.includes("Cẩm nang Hỗ trợ"), {
      timeout: 8000,
    });
  });
  ...
  await check(page, "Trang My KPI hiển thị các chỉ số", async () => {
    await page.waitForFunction(
      () =>
        document.body.innerText.includes("TỔNG TICKET") ||
        document.body.innerText.includes("MY.KPI"),
      { timeout: 10000 }
    );
  });
  ...
  await check(page, "Trang bảo trì hiển thị lịch và danh sách ticket", async () => {
    await page.waitForFunction(
      () =>
        document.body.innerText.includes("Lịch bảo trì") ||
        document.body.innerText.includes("Bảo trì"),
      { timeout: 10000 }
    );
  });
  ...
  await check(page, "Trang lịch sử chat AI load", async () => {
    await page.waitForFunction(
      () => document.body.innerText.includes("Lịch sử") || document.body.innerText.includes("Chat"),
      { timeout: 10000 }
    );
  });
  ...
  await check(page, "Trang Cài đặt hiển thị mục Bảo mật 2 lớp", async () => {
    await page.waitForFunction(
      () =>
        document.body.innerText.includes("bảo mật") ||
        document.body.innerText.includes("2-FA") ||
        document.body.innerText.includes("CÀI ĐẶT"),
      { timeout: 10000 }
    );
  });
