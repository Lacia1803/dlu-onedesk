...
describe("Rate limiter — chatbot-style (10 req/min)", () => {
  ...
  it("allows first 10 requests, blocks the 11th", () => {
  ...
  it("different keys are independent", () => {
    ...
    for (let i = 0; i < 10; i++) {
      assert.strictEqual(rateLimit(key1, 10, 60_000).allowed, true);
    }
    assert.strictEqual(rateLimit(key1, 10, 60_000).allowed, false);
    assert.strictEqual(rateLimit(key2, 10, 60_000).allowed, true);
  });

  it("resets after window expires", async () => {
    const key = "chatbot:window-test";
    const { allowed } = rateLimit(key, 2, 50); // 50ms window
    assert.strictEqual(allowed, true);
    assert.strictEqual(rateLimit(key, 2, 50).allowed, true);
    assert.strictEqual(rateLimit(key, 2, 50).allowed, false);
...
describe("Rate limiter — register-style (5 req/min)", () => {
  ...
  it("blocks after 5 rapid registrations from same IP", () => {
    const key = "register:192.168.1.1";
    for (let i = 0; i < 5; i++) {
      assert.strictEqual(rateLimit(key, 5, 60_000).allowed, true);
    }
    assert.strictEqual(rateLimit(key, 5, 60_000).allowed, false);
  });
});
...
describe("Chatbot rate-limit integration", () => {
  ...
  it("returns rate-limit message after 10 rapid calls for same user", () => {
    const userId = "user-123";
    const key = `chatbot:${userId}`;
    for (let i = 0; i < 10; i++) {
      assert.strictEqual(rateLimit(key, 10, 60_000).allowed, true);
    }
    assert.strictEqual(rateLimit(key, 10, 60_000).allowed, false);
  });

  it("anonymous users share 'anon' key correctly", () => {
    const key = "chatbot:anon";
    for (let i = 0; i < 10; i++) {
